...
private
 ...
 contains
  procedure :: set_r
  procedure :: set_area => circle_area
  procedure :: set_perimeter => circle_circumfrence
...
public :: pi, shape_type, rectangle_type, square_type, circle_type ! List only what needs to be public
...
subroutine set_r (this, r)
  ...
  real :: r
  this%radius = r
  this%area = this%set_area()
  this%perimeter = this%set_perimeter()
end subroutine set_r
real function circle_area (this)
  class(circle_type) :: this
  circle_area = this%radius**2.0 * pi
end function circle_area
real function circle_circumfrence (this)
  class(circle_type) :: this
  circle_circumfrence = 2.0 * pi * this%radius
end function circle_circumfrence
...
subroutine shape_init (this, v1, v2)
 class(shape_type), intent(inout), target :: this
 real, intent (in) :: v1 
 real, intent (in) :: v2
 class(shape_type) , pointer :: unknown_shape !! This can be used for any shape
  ...
  select type (unknown_shape)
    type is (rectangle_type)
      call unknown_shape%set_l_w(v1, v2)
    type is (square_type)
      call unknown_shape%set_l(v1)
    type is (circle_type)
      call unknown_shape%set_r(v1)
    class default
...
type(square_type) :: square !! This is a square
type(rectangle_type) :: rectangle 
type(circle_type) :: unit_circle
type(rectangle_type) :: rec2
type(circle_type) :: circle

 call unit_circle%shape_init(1.0, 0.0)
 call square%shape_init(1.0, 0.0)
 call circle%shape_init(0.5, 0.0)
 write (6,*) "Percentage of square that is not circle"
 write (6,*) ( square%get_area() - circle%get_area() ) / square%get_area()
